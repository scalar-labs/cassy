...
result=$(grpcurl -plaintext -d '{"cluster_id": "'"$cluster_id"'", "snapshot_id": "'"$snapshot_id"'", "backup_type": 3}' localhost:20051 rpc.Cassy.TakeBackup)

status=$(echo $result | jq -r '.status')
if [[ $status != "STARTED" ]]; then
  	send_message_slack "Incremental backup creation failed"
  	exit 1
fi
...
send_message_slack "Incremental backup successfully created."
