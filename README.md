# Cassy: Simple and integrated backup tool for Apache Cassandra

Cassy is a simple and integrated backup tool for Apache Cassandra.

You can do the following things with Cassy from an easy to use gRPC API or HTTP/1.1 REST API:
...
The existing Cassandra backup features such as snapshot and incremental backup are great building blocks for doing backup and restore for Cassandra data. However they are not necessarily easy to use because it is not fully integrated. Moreover, the current backup feature or the existing backup tools are problematic when it is used with [Scalar DB](https://github.com/scalar-labs/scalardb/) transaction that updates multiple records in a transactional (atomic) way because they do not handle transactional consistency of multiple records. 
This is why we come up with a new backup tool, which can not only do backup and restore in an easy and integrated manner, but also take a cluster-wide transactionally consistent backup.
...
This section briely describes how to install and use Cassy.
...
Of-course, you need a well-configured Cassandra cluster that you can manage. The following Cassandra configrations are required to be updated to run Cassy properly.
...
* Place a SSH private key withou passphase in the master node and a corresponding public key to each Cassandra node. 
* Configure AWS config and credential properly. (Note that the first version only supports AWS S3 as a blob store) 

### Configure

To run Casssy propely, it is required to create a property file.
...
# Username for SSH.  
scalar.backup.cassandra.server.ssh_user=hiro
...
Let's start a Cassy master.

```
$ build/install/cassandra-backup/bin/backup-server --config ./backup-server.properties
```

Now you can run backup and restore through gRPC API or HTTP/1.1 REST API. For gRPC API, you can do it easily with [grpcurl](https://github.com/fullstorydev/grpcurl).
...
```
You need to specify an address of one of Cassandra nodes to register your cluster. All the cluster information such as cluster name, nodes' IP addresses, keyspaces and the data directory are saved in the metadata database.

Note that you need to re-register your cluster if you update some of the above cluster information. Backup files with an old cluster information will be invalid in such case.
...
You can take a snapshot of each node in a specified cluster and upload the snapshots to the blob store with `TakeBackup`.
...
```
`"backup_type": 2` means taking a snapshot. Please specify `"backup_type": 3` when you want to upload incremental backups.

```
# It asks each node to upload incremental bakcups to the specified location.
$ grpcurl -plaintext -d '{"cluster_id": "", "snapshot_id": "", "backup_type": 3}' 192.168.0.254:20051 rpc.CassandraBackup.TakeBackup 
```

You need to specify `snapshot_id` to upload incremental backups properly because those backups are differences from a previous snapshot.
...
Cassy master asks a DNS to resolve the service URL to get a list of IPs and ports of Cassandra applications, and ask those nodes to pause. The node that is asked to pase will finish already received requests and will be in a pause mode afterward and return a response to Cassy master. The master waits for all responses for pause, and then the master takes snapshots of all Cassandra nodes. Then the master unpause the application nodes and upload all the snapshots.

Note that applications must have pause feature to do this. Cassy currently supports gRPC endpoints for pause. For more detail, please take a look at the source code.

If the cluster-wide backup is take during a non-busy time, the paused duration will be very short like one second, and it won't drop any requests if applications queue requests proplery during pause.
...
Note that there should be no data, commitlog, hints in a restored node.

```
$ grpcurl -plaintext -d '{"snapshot_id": "", "restore_type": 2, "cluster_id": ""}' 192.168.0.254:20051 rpc.CassandraBackup.RestoreBackup
```

It will download the specified snapshot and the incremental backups from the snapshot into Cassandra data directory with a directory named `node-backup` and place the files properly for Cassandra to start up with the files.
...
* GUI component to make backup and restore operation even easiler.
